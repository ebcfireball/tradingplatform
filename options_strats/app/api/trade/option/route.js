...
export async function POST(req) {
    ...
    try {
        ...
        for (let opt of userInfo.optiontrade) {
            if (opt.contractSymbol == contractSymbol && opt.type != type) {
                const opttrade = await prisma.optiontrade.delete({
                    where: {
                        type: opt.type,
